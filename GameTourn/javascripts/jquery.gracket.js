...
(function($) {
  $.fn.gracket = function(method) {
        ...
        for (var r=0; r < round_count; r++) {
          ...
          for (var g=0; g < game_count; g++) {
            ...
            for (var t=0; t < team_count; t++) {
              ...
              var team_html = helpers.build.team(data[r][g][t], this.gracket.settings, team_count);
    ...
    var helpers = {
      build : {
        team : function(data, node, team_count){
          var html = [
						(team_count === 1) ? '<span class="' + node.winnerTxtClass + '">WINNER</span>' : '',
            '<h3'+ ((typeof data.score === "undefined") ? "" : " title=\"Score: " + data.score + "\"") +'>',
							'<i class="' + (data.winner ? "fa fa-check" : "fa fa-times") + '"></i>',
							'<img class="' + node.logoClass + '" src="' + data.logo + '" alt="' + data.name + '" />',
              '&nbsp;' + data.name + '&nbsp;',
              '<small>',
